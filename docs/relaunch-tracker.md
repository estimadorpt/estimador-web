...
**Post-launch design pass.** The live graphic assessment (`design/live-graphic-assessment-2026-09-11.md`) was implemented the same day: phone atlas frame that slices the world instead of letterboxing it, compact product heroes with colour fields, darker supporting text (faint step 4.6:1), a dated football lead under the homepage village with a compact paused-economy line, a close village crop and the compact wordmark on phones. Details in that document's follow-up section.

**Homepage rebuilt (handoff 2B).** Implemented `design/homepage-claude-handoff/IMPLEMENTATION.md`: population lead with the cutaway, football rail with the dated title probabilities from the latest matchday, economy and elections as equal support panels with their own illustrations, a typed standard/election mode in `src/lib/config/homepage.ts` (tested, with a no-data fallback and an env preview), archive-accurate election labels, responsive AVIF/WebP art, verified at 1440/1024/768/390/320 in both languages on the fresh export. Not deployed: the handoff asked for verification only.
