  ...
  return (
    <div className={`relative ${className}`}>
      <button
        ...
        className="flex min-h-11 items-center gap-2 px-4 py-2 bg-cream border border-stone-300 rounded-[10px] hover:bg-parchment transition-colors min-w-[280px] text-left"
      >
        <Icon className="w-4 h-4 text-green-600" />
        <div className="flex-1">
          <div className="font-medium text-stone-900">{currentElection.name}</div>
      ...
      {isOpen && (
        <div className="absolute top-full left-0 right-0 mt-1 bg-cream border border-stone-300 rounded-lg shadow-lg z-50 max-h-[400px] overflow-y-auto">
          {Object.entries(groupedElections).map(([type, elections]) => {
            ...
            return (
              <div key={type} className="border-b border-stone-100 last:border-b-0">
                <div className="px-4 py-2 bg-stone-50 text-sm font-medium text-stone-700 flex items-center gap-2">
                ...
                {elections.map(election => (
                  ...
                  >
                    <div className="font-medium text-stone-900">{election.name}</div>
                    <div className="text-sm text-stone-500 flex items-center gap-2">
                      ...
                      {!election.isActive && new Date(election.date) > new Date() && (
                        <span className="px-2 py-0.5 bg-blue-100 text-ink-dark text-xs rounded-full">
                          Prevista
                        </span>
                      )}
                      {!election.isActive && new Date(election.date) < new Date() && (
                        <span className="px-2 py-0.5 bg-stone-100 text-stone-800 text-xs rounded-full">
                          Histórica
                        </span>
                      )}
                    </div>
                    {election.description && (
                      <div className="text-xs text-stone-400 mt-1">{election.description}</div>
                    )}
                  </button>
                ))}
              </div>
