...
export function LocalMap({locale,region,municipality,parish,onRegion,onMunicipality,onParish,onBack,cohort,lens,paused,selectedHouse,selectedPerson,onHouse}:{locale:'pt'|'en';region:string;municipality:string;parish:string;onRegion:(name:string)=>void;onMunicipality:(code:string)=>void;onParish:(code:string)=>void;onBack:()=>void;cohort:Cohort;lens:Lens;paused:boolean;selectedHouse:number|null;selectedPerson:number|null;onHouse:(parish:string,id:number)=>void}) {
 ...
 // Phones show the 1000×620 world through a portrait frame. Instead of letterboxing
 // it (a small mainland under empty bands) the frame slices it, and every measure
 // that assumed the frame's width spans the world is scaled by what is visible.
 const [frameRatio,setFrameRatio]=useState(1000/620);
 useEffect(()=>{const svg=svgRef.current;if(!svg)return;const observer=new ResizeObserver(([entry])=>{const {width,height}=entry.contentRect;if(width&&height)setFrameRatio(width/height);});observer.observe(svg);return()=>observer.disconnect();},[]);
 const slice=compact,visibleFactor=slice?Math.min(1,.62*frameRatio):1;const sliceRef=useRef(slice);sliceRef.current=slice;
 const fitView=useCallback((bounds:[[number,number],[number,number]])=>{const view=fitCamera(bounds);if(!slice)return view;const [[x0,y0],[x1,y1]]=bounds;return {...view,width:Math.max((x1-x0)/visibleFactor,(y1-y0)/.62)*1.34};/* more room than on wide screens: the caption sits above the place, the journey scale below it */},[slice,visibleFactor]);
 ...
 const geographicView=useMemo(()=>{
  if(region==='Portugal'&&country)return fitView(worldPath.bounds(countryExtent(country,fullExtent)));
  if(parish){if(!selectedScene||!selectedFeature)return null;return fullExtent?fitView(worldPath.bounds(selectedFeature)):{...selectedScene.view,width:selectedScene.view.width/visibleFactor};}
  if(municipality){const f=districts?.features.find(f=>f.properties.code===municipality);return f?fitView(worldPath.bounds(fullExtent?f:mainExtent(f))):null;}
  return districts&&district&&overview[district.regionId]?fitView(worldPath.bounds(districts)):null;
 },[region,country,parish,selectedScene,selectedFeature,fullExtent,municipality,districts,district,overview,fitView,visibleFactor]);
 useEffect(()=>{const svg=svgRef.current;if(!svg)return;const wheel=(event:WheelEvent)=>{if(!event.ctrlKey&&!event.metaKey)return;const current=cameraRef.current;if(!current)return;event.preventDefault();const box=svg.getBoundingClientRect(),pixelScale=(sliceRef.current?Math.max:Math.min)(box.width/1000,box.height/620);const width=Math.max(.5,Math.min(15000,current.width*Math.exp(event.deltaY*.01))),factor=width/current.width;const dx=(event.clientX-box.left-box.width/2)/pixelScale/1000*current.width,dy=(event.clientY-box.top-box.height/2)/pixelScale/1000*current.width;setManualView({x:current.x+dx*(1-factor),y:current.y+dy*(1-factor),width});};svg.addEventListener('wheel',wheel,{passive:false});return()=>svg.removeEventListener('wheel',wheel);},[]);
 const target=useMemo(()=>{
  ...
  return {x:home?.x??view.x,y:home?home.y-selectedScene!.unit*18:view.y,width:(home?selectedScene!.unit*1000/2.8/visibleFactor:view.width)/zoom};
 },[geographicView,selectedScene,selectedHouse,zoom,manualView,reset,visibleFactor]);
 useEffect(()=>{
  ...
  const tick=(now:number)=>{if(drag.current?.moved){setFlying(false);return;}const t=Math.min(1,(now-start)/duration),eased=t*t*(3-2*t);const [x,y,width]=interpolation(eased);const next={x,y,width};cameraRef.current=next;const transform=cameraTransform(next);if(cameraGroup.current)cameraGroup.current.style.transform=`translate(${transform.x}px,${transform.y}px) scale(${transform.scale})`;coastalGroup.current?.setAttribute('opacity',String(coastalOpacity(width*visibleFactor)));overviewGroup.current?.setAttribute('opacity',String(householdOverviewOpacity(width*visibleFactor)));frames++;maxGap=Math.max(maxGap,now-previousFrame);previousFrame=now;if(t<1)frame=requestAnimationFrame(tick);else{setCamera(next);setFlying(false);if(cameraGroup.current){cameraGroup.current.dataset.flightFrames=String(frames);cameraGroup.current.dataset.flightMaxGapMs=String(Math.round(maxGap));}}};
 ...
 <svg ref={svgRef} viewBox="0 0 1000 620" preserveAspectRatio={slice?'xMidYMid slice':'xMidYMid meet'} style={{touchAction:zoom>1?'none':'pan-y'}} onPointerDown={event=>{if(event.button===0&&camera)drag.current={x:event.clientX,y:event.clientY,camera,moved:false};}} onPointerMove={event=>{const start=drag.current;if(!start)return;const dx=event.clientX-start.x,dy=event.clientY-start.y;if(Math.abs(dx)+Math.abs(dy)<5&&!start.moved)return;start.moved=true;event.currentTarget.setPointerCapture(event.pointerId);const box=event.currentTarget.getBoundingClientRect(),ratio=start.camera.width/(1000*(sliceRef.current?Math.max:Math.min)(box.width/1000,box.height/620));const next={...start.camera,x:start.camera.x-dx*ratio,y:start.camera.y-dy*ratio};cameraRef.current=next;const transform=cameraTransform(next);if(cameraGroup.current)cameraGroup.current.style.transform=`translate(${transform.x}px,${transform.y}px) scale(${transform.scale})`;}} onPointerUp={()=>{if(cameraRef.current)setCamera(cameraRef.current);setTimeout(()=>{drag.current=null;},0);}} onPointerCancel={()=>{drag.current=null;}} onClickCapture={event=>{if(drag.current?.moved){event.preventDefault();event.stopPropagation();}}} aria-label={pt?'Portugal: viagem contínua até às casas':'Portugal: continuous journey into homes'}>
 ...
 <g ref={coastalGroup} opacity={coastalOpacity(camera.width*visibleFactor)}><g pointerEvents="none">{waters.map(({code,d})=><g key={code}><path d={d} fill="#245c68"/><path className="atlas-water-surface" d={d} fill="url(#world-water)"/></g>)}</g>
 ...
 {scenes.filter(scene=>scene.code===parish||[camera,target].some(view=>view&&scene.unit*1000/view.width>.38&&Math.abs(scene.view.x-view.x)<(scene.view.width+view.width)*.8&&Math.abs(scene.view.y-view.y)<(scene.view.width+view.width)*1.2)).map(scene=><ParishLandscape key={scene.code} scene={scene} profile={scene.profile} selected={scene.code===parish} selectedHouse={selectedHouse} selectedPerson={selectedPerson} cohort={cohort} lens={lens} pt={pt} interactive={scene.code===parish} detailed={scene.code===parish&&scene.unit*transform.scale/visibleFactor>.38} onHouse={onHouse}/>)}
 <g ref={overviewGroup} opacity={householdOverviewOpacity(camera.width*visibleFactor)} pointerEvents="none" className="atlas-household-overview">{overviewPaths.filter(p=>!scenes.some(s=>s.code===p.code&&s.unit*transform.scale/visibleFactor>.38)).map(p=><path key={p.code} data-overview-parish={p.code} data-households={p.count} d={p.d} fill="none" stroke="#d9c397" strokeWidth="2.6" strokeLinecap="round" vectorEffect="non-scaling-stroke"/>)}</g>
