...
const KIND_STYLE: Record<BadgeKind, { border: string; text: string; dot: string }> = {
  reading: { border: 'border-[#245c68]/30', text: 'text-[#245c68]', dot: 'bg-[#245c68]' },
  ...
  officialCalc: { border: 'border-[#245c68]/30', text: 'text-[#245c68]', dot: 'bg-[#245c68]' },
...
export function StatusBadge({
  ...
  return (
    <Link
      ...
      title={title}
      className={`inline-flex items-center gap-1 rounded-full border bg-cream px-2 py-0.5 text-[11px] font-bold uppercase tracking-wider no-underline hover:opacity-80 transition-opacity ${s.border} ${s.text}`}
    >
