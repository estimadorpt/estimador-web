...
k(t, t') = σ² × exp(-|t-t'|² / (2ℓ²))
```
where σ is the amplitude and ℓ is the lengthscale (~1460 days).
...
House effects are modeled with a zero-sum constraint: if a pollster overestimates one party, they must underestimate other(s) to compensate.
...
These offsets are estimated from previous election results and allow predicting how national vote shares translate into district-level votes.
...
The innovation standard deviation (σ ≈ 0.05 log-odds/day) is calibrated to allow movements of ~5 percentage points over a 50-day campaign.
